...
ConfigurationClient.prototype.listForNamespace = function(namespace, options, callback) {
  var url = sprintf('/configuration'), tmp = namespace;
...
function Client(username, apiKey, region, options) {
  options = options || {};
  var authUrl = utils.getAuthUrl(options, region);

  if (!options.keystoneClient) {
    options.keystoneClient = new KeystoneClient(authUrl, {'username': username,
                                                          'apiKey': apiKey,
                                                          'cacheTokenFor': 600});
  }
