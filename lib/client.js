...
function HeartBeater(url, tenantId, token, options, sessionId, initialToken, timeout) {
  this._sessionId = sessionId;
  this._heartbeatTimeout = timeout;
  this._heartbeatInterval = (this.heartbeatTimeout * 0.6);
  this._timeoutId = null;
  this._nextToken = initialToken;
  HeartBeater._super.call(this, url, tenantId, token, options);
}
...
HeartBeater.prototype._startHeartbeating = function() {
  ...
  this._request(url, 'POST', payload, {'expectedStatusCode': 200}, function(err, res) {
    ...
    self._timeoutId = setTimeout(self._startHeartbeating.bind(self), self._heartbeatInterval);
  });
};
