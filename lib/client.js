...
function makeRequest(url, method, payload, options, callback) {
  method = method || 'GET';
  payload = payload || null;
  options = options || {};
  var extraHeaders = options.headers || {}, expectedStatusCode = options.expectedStatusCode,
       reqOptions, defaultHeaders, headers, curlCmd;
...
BaseClient.prototype._request = function(path, method, payload, options, callback) {
  ...
  makeRequest(url, method, payload, options, callback);
};
...
EventsClient.prototype.list = function(sinceToken, options, callback) {
  ...
  if (sinceToken !== null && sinceToken !== undefined) {
    options.queryString = {'sinceToken': sinceToken};
  }
  url = sprintf('/events');
  this._list(url, options, callback);
};
...
ServicesClient.prototype.join = function(serviceId, sessionId, payload, callback) {
  payload = payload || {};
  payload.session_id = sessionId;
  var url = sprintf('/services/%(serviceId)s', {'serviceId': serviceId});
  this._create(url, payload, {}, callback);
};

ServicesClient.prototype.leave = function(serviceId, sessionId, callback) {
  var url = sprintf('/services/%(serviceId)s', {'serviceId': serviceId});
...
/* HeartBeater */
function HeartBeater(baseUrl, tenantId, authToken, sessionId, initialToken, timeout) {
  this._baseUrl = url + tenantId;
  this._authToken = authToken;
  this._sessionId = sessionId;
  this._heartbeatTimeout = timeout;
  this._heartbeatInterval = (this.heartbeatTimeout * 0.6);
  this._timeoutId = null;
  this._nextToken = initialToken;
}

HeartBeater.prototype._startHeartbeating = function(sessionId, heartbeatTimeout, heartbeatInterval, timeoutId, nextToken) {
  var url = sprintf('%(baseUrl)s/sessions/%(sessionId)s/heartbeat',
      {'sessionId': this._sessionId, 'baseUrl': this._baseUrl}),
      payload = {'token': this._nextToken},
      options = {'expectedStatusCode': 200,
                 'headers': {'X-Auth-Token': this._authToken}};

  makeRequest(url, 'POST', payload, options, function(err, res) {
    if (err) {
      callback(err);
      return;
    }

    this.nextToken = res.body.token;
    setTimeout(this._startHeartbeating, this._heartbeatInterval);
  });
};
