...
SessionsClient.prototype.create = function(heartbeatTimeout, payload, callback) {
  ...
  this._create(url, payload, {}, function(err, res) {
    ...
    hb = new HeartBeater(self._url, self._tenantId, self._token, self._options,
                         sessionId, initialToken, heartbeatTimeout);

    callback(null, res.headers.location, res.body, hb);
  });
};
...
function HeartBeater(url, tenantId, token, options, sessionId, initialToken, timeout) {
  ...
  HeartBeater.super_.call(this, url, tenantId, token, options);
}
...
HeartBeater.prototype._startHeartbeating = function() {
  ...
  this._request(url, 'POST', payload, reqOptions, function(err, res) {
    ...
    log.debug('Scheduling next interval for ${interval} ms in future',
              {'sessionId': self._sessionId, 'interval': interval});
